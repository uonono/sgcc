...
public interface RepairRecordRepository extends ReactiveCrudRepository<RepairRecord, Long> {
     ...
     */
    @Query("""
            SELECT r.*, e.created_at FROM repair_record r LEFT JOIN fault_order fo ON
             fo.id = r.fault_order_id LEFT JOIN evaluation e ON r.fault_order_id = e.fault_order_id
             WHERE r.fault_order_id = :faultOrderId AND fo.openid = :openid
            """)
    Mono<RepairRecordWithCreatedAt> findByFaultOrderId(Long faultOrderId,String openid);
